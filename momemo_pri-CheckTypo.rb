### Typo check for pri mom emotion coding.

## Parameters
map_mom = { 'momemo' => { 'emotion_pn' => %w(p n .) } }

cell_thresh = 167 #threshold for between coded cells

#list of all "codes" within ID column
id_map = {'emo_id_mom' => %w[lab_id momemo_coder momemo_date momemo_mins]}
...
puts "Wrong Duration:"
momemo=getColumn("momemo")
prevcell=nil #ref to previous col

# Loop over all the emotion cells
momemo.cells.each do |momemocell|
  ordinal = momemocell.ordinal.to_i
  onset = momemocell.onset.to_i
  offset = momemocell.offset.to_i
  ...
  if (!prevcell.nil? && onset < prevcell.offset.to_i)
    ...
    if (!prevcell.nil? &&
    prevcell.emotion_pn==momemocell.emotion_pn &&
    (momemocell.onset-prevcell.offset).abs<cell_thresh)

    puts("CELLS #{prevcell.ordinal} and #{momemocell.ordinal} are emotion bouts with less than 5 frames interval")
  end
...
#Check for typo in the lab_id under emo_id_mom
